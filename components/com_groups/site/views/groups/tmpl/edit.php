							...
							<ul id="access">
								...
								<?php for ($i=0; $i<count($this->hub_group_plugins); $i++): ?>
									<?php if ($this->hub_group_plugins[$i]['display_menu_tab']): ?>
										<li class="group_access_control_<?php echo strtolower($this->hub_group_plugins[$i]['name']); ?>">
											...
											<span class="menu_item_title">
												<?php 
												$pluginKey = 'COM_GROUPS_' . strtoupper($this->hub_group_plugins[$i]['name']);
												echo Lang::txt($pluginKey); 
												?>
											</span>
